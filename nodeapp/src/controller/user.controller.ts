...
export const getUsers = async (req: Request, res: Response): Promise<Response<HttpResponse>> => {
    ...
    try{
        ...
        console.error(error);
        return res.status(Code.INTERNAL_SERVER_ERR)
            .send(new HttpResponse(Code.INTERNAL_SERVER_ERR, Status.INTERNAL_SERVER_ERR, 'An error occurred'));
    }
...
export const getUser = async (req: Request, res: Response): Promise<Response<HttpResponse>> => {
    ...
    try{
        ...
        const pool = await connection();
        const result: ResultSet = await pool.query(QUERY.SELECT_USERS_BY_NAME, [req.params.userName]);
...
export const createUser = async (req: Request, res: Response): Promise<Response<HttpResponse>> => {
    ...
    try{
        ...
        user = { userName: (result[0] as ResultSetHeader).insertId, ...req.body};


            return res.status(Code.CREATED)
                .send(new HttpResponse(Code.CREATED, Status.CREATED, 'User created', result[0]));
...
export const updateUser = async (req: Request, res: Response): Promise<Response<HttpResponse>> => {
    ...
    try{
        ...
        if((result[0] as Array<ResultSet>).length > 0){
            ...
            const result: ResultSet = await pool.query(QUERY.UPDATE_USER, [...Object.values(user), req.params.userName]);
...
export const deleteUser = async (req: Request, res: Response): Promise<Response<HttpResponse>> => {
    ...
    try{
        ...
        const pool = await connection();
        const result: ResultSet = await pool.query(QUERY.SELECT_USERS_BY_NAME, [req.params.userId]);
